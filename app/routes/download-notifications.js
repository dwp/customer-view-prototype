...
router.all('/get-a-proof-of-benefit-letter/download/download-notifications/research-set-up/PoB-clear-data', function (req, res) {
  ...
  router.post('/get-a-proof-of-benefit-letter/download/download-notifications/research-set-up/benefits-answer', function (req, res) {
  ...
  res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/research-set-up/address');
...
router.post('/get-a-proof-of-benefit-letter/download/download-notifications/how-did-you-find-out-about-this-service-answer', function (req, res) {
  ...
  if (howDidYouFindOutAboutThisService === "heard-about-on-phone" || howDidYouFindOutAboutThisService === "job-centre")  {
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/go-to-identity-check');

  } else {
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/cannot-get-a-proof-of-benefit-letter');
  }
})

// Select which benefit selection page to show based on number of benefits available for user.
router.all('/get-a-proof-of-benefit-letter/download/download-notifications/list-benefits-answer', function (req, res) {

  var researchSetUpBenefits = req.session.data['researchSetUpBenefits']

  if (researchSetUpBenefits == [] || researchSetUpBenefits == undefined)  {
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/you-cannot-use-this-service-no-benefits');
    return

  } else if ((researchSetUpBenefits.length > 0) && (inScope.length > 0 ))   {
      res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/select-benefits-you-need-proof-of');

  } else if (inScope.length === 0 )  {
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/you-cannot-get-proof-of-benefit-letter');

  } 
  
})

router.post('/get-a-proof-of-benefit-letter/download/download-notifications/select-benefits-answer', function (req, res) {

  var doYouWantLetterFor = req.session.data['doYouWantLetterFor']

    if (doYouWantLetterFor === 'no') {
      res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/contact-us-for-different-benefit-letter');
    } 

    else {
      res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/where-we-send-your-letter');
    }
...
router.post('/get-a-proof-of-benefit-letter/download/download-notifications/send-letter-to-address-answer', function (req, res) {
  ...
  if (confirmLetterSend == "no")  {
    ...
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/you-cannot-use-this-service-incorrect-address');
  }

  else {
    // Send user to check answers
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/digital-or-letter');
  }

})
//On check answers page, if selected letter, send to conf, if not, send to download 
router.post('/get-a-proof-of-benefit-letter/download/download-notifications/check-your-answers-answer', function (req, res) {

  var digitalOrLetter = req.session.data['digitalOrLetter']

  if (digitalOrLetter === 'Posted letter') {
    res.redirect('/get-a-proof-of-benefit-letter/download/download-notifications/request-complete');
